...
const CompetitionForm = ({ onSubmit, initialData = {} }) => {
  ...
  const fetchTeamMembers = async (searchTerm = "") => {
    try {
      const response = await axios.get(`${API}/api/admin/team?search=${searchTerm}`,{withCredentials:true});
  ...
  const handleTeamMemberInputChange = async (e) => {
    ...
    if (value.length > 1) {
      try {
        const response = await axios.get(`${API}/api/admin/team?search=${value}`,{withCredentials:true});
