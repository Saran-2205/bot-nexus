...
const fetchCompetitions = async () => {
  const res = await fetch(`${API}/api/admin/competitions/`, { credentials: "include" });
      ...
      >
        {isLoading ? (
          ...
          Array.from({ length: 6 }).map((_, i) => (
            <AdminCard key={`skeleton-${i}`} isLoading skeletonLines={2} />
          ))
        ) : isError ? (
          ...
          competitions?.map((competition) => (
            <AdminCard
              ...
              image={competition.heroImg}
              editLink={`/nexus-hq/competitions/edit/${competition.slug}`}
