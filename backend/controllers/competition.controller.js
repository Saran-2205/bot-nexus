  ...
  try {
    ...
    if (
      ...
      (!Array.isArray(overview) ||
        overview.some((item) => typeof item !== "string"))
    ) {
    ...
    Object.keys(competitionData).forEach((key) => {
      if (
        Array.isArray(competitionData[key]) &&
        competitionData[key].length === 0
      ) {
...
export const getCompetitionByParams = async (req, res) => {
  ...
  try {
    ...
    if (!competition) {
      competition = await Competition.findOne({ slug: param }).populate(
        "teamMembers"
      );
    }
...
export const updateCompetition = async (req, res) => {
  try {
    ...
    // Validate overview as array of strings
    if (
      overview !== undefined &&
      (!Array.isArray(overview) ||
        overview.some((item) => typeof item !== "string"))
    ...
    const validateKeyValueArray = (arr, name) => {
      if (
        ...
        errors.push(
          `${name} must be an array of objects with string key and value`
        );
      }
    };
    ...
    const validateTechArray = (arr) => {
      if (
        arr &&
        (!Array.isArray(arr) ||
          arr.some(({ title }) => typeof title !== "string"))
      ) {
        errors.push(
          "keyTechnologies must be an array of objects with string title"
        );
      }
    };
    ...
    if (teamMembers) {
      if (!Array.isArray(teamMembers)) {
        ...
        const areValidIds = teamMembers.every((id) =>
          mongoose.Types.ObjectId.isValid(id)
        );
        if (!areValidIds) {
