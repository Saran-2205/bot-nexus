...
export const createProject = async (req, res) => {
  try {
    ...
    // Upload hero image if not from Cloudinary
    if (img && !img.startsWith("https://res.cloudinary.com/")) {
      const uploadedResponse = await cloudinary.uploader.upload(img);
      img = uploadedResponse.secure_url;
    }

    // Overview array validation
    if (
      overview &&
      (!Array.isArray(overview) || overview.some((p) => typeof p !== "string"))
    ) {
      return res.status(400).json({ error: "Overview must be an array of strings." });
    }
    const cleanedOverview = Array.isArray(overview) ? overview.map((p) => p.trim()) : [];
    ...
    if (
      coreObjectives &&
      (!Array.isArray(coreObjectives) || coreObjectives.some((obj) => typeof obj !== "string"))
    ) {
      return res.status(400).json({ error: "Core Objectives must be an array of strings." });
    }
    ...
    if (
      technologies &&
      (!Array.isArray(technologies) || technologies.some((tech) => typeof tech !== "string"))
    ) {
      return res.status(400).json({ error: "Technologies must be an array of strings." });
    }
    ...
    if (Array.isArray(team) && team.length > 0) {
      const areAllValidObjectIds = team.every((id) => mongoose.Types.ObjectId.isValid(id));
      if (!areAllValidObjectIds) {
        return res.status(400).json({ error: "One or more team IDs are not valid ObjectIds." });
      }
      const validTeamMembers = await Team.find({ _id: { $in: team } });
      if (validTeamMembers.length !== team.length) {
        return res.status(400).json({ error: "Provide valid team member IDs." });
      }
    }
...
export const getProjectByParams = async (req, res) => {
  ...
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(param)) {
      project = await Project.findById(param).populate("team", "_id name designation image");
    }
    if (!project) {
      project = await Project.findOne({ slug: param }).populate("team", "_id name designation image");
    }
    if (!project) {
...
export const updateProject = async (req, res) => {
  try {
    ...
    if (shortDesc && typeof shortDesc !== "string")
      return res.status(400).json({ error: "Short description must be a string." });
    ...
    const cleanedOverview =
      Array.isArray(overview) ? overview.map((p) => p.trim()) : undefined;
    ...
    if (
      coreObjectives &&
      (!Array.isArray(coreObjectives) || coreObjectives.some((item) => typeof item !== "string"))
    )
      return res.status(400).json({ error: "Core Objectives must be an array of strings." });
    ...
    if (
      ...
      return res.status(400).json({
        error: "Technical specifications must be an array of objects with string 'key' and 'value'.",
      });
    ...
    if (
      ...
      return res.status(400).json({
        error: "Each milestone must have a string title, string description, and a valid date.",
      });
    ...
    const updates = {
      ...
      ...(shortDesc && { shortDesc }),
      ...(img && { img }),
      ...(cleanedOverview && { overview: cleanedOverview }),
      ...(desc && { desc }),
